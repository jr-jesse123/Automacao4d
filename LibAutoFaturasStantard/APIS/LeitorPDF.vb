...
#Enable Warning BC42104 ' Variável "ddregex" é usada antes de receber um valor. Uma exceção de referência nula poderia resultar em runtime.
        ...
        For Each padrao In Regexer.Padroes
            ...
            Catch ex As Exception
                Throw New PdfCorrompidoException(fatura, "falha ao realizar REGEX")
            End Try
        ...
        If conta.Operadora = OperadoraEnum.VIVO Then
            ...
            ElseIf conta.TipoDeConta = TipoContaEnum.FIXA Then
                ...
                ElseIf conta.Subtipo = SubtipoEnum.VozFixa Then
                    ...
                    texto, $"(Número do telefone: (.+)?({conta.NrDaConta})(.+)?\b)|\b\d{10}\b", RegexOptions.Multiline)
                    Dim output
                    If teste.Value <> "" Then

                        For Each group As Group In teste.Groups
                            If group.Value.Length = 10 Then
                                output = group.Value
                                Exit For
                            End If
                        Next
    ...
    Private Function VerficarReferenciaDaFatura(texto As String, fatura As Fatura) As String
        ...
        If conta.Operadora = OperadoraEnum.VIVO Then
            If conta.TipoDeConta = TipoContaEnum.MOVEL Then
                ...
                Dim rawRef = Regex.Match(texto, "Referência:? (\d{2}/\d{4})").Groups(1).Value
